...
### Changed
- **The root `/` is now a public homepage** for logged-out visitors (the login box is part of it, revealed in place). It describes the app's purpose and features, **exactly what Google Health data is requested (read-only daily step count) and how it's used** (converted to miles for journey progress; never sold/shared/advertised; optional and disconnectable), and links to the Privacy Policy and Terms. This addresses the verification flags about the homepage being behind login, not explaining the app or its use of Google data, and not linking to privacy. The homepage URL for the consent screen is the site root.
