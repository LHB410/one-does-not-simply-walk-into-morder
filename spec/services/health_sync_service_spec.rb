...
RSpec.describe HealthSyncService do
  include_context "user with path progress"

  let(:service) { described_class.new(user) }

  before do
    user.update!(health_uid: "TESTUID", health_access_token: "token123")
  ...
  def stub_health_steps(steps)
    allow_any_instance_of(HealthClient).to receive(:fetch_steps).and_return(steps)
  end

  describe "#call" do
    context "when user has no prior steps" do
      before { stub_health_steps(8000) }

      it "records the full step count" do
        expect { service.call }.to change { user.step.reload.total_steps }.by(8000)
      end

      it "sets steps_today to the synced total" do
        service.call
      ...
      it "sets health_last_sync_at" do
        expect { service.call }.to change { user.reload.health_last_sync_at }.from(nil)
      end
    ...
    context "when user already entered steps manually today" do
      ...
      it "replaces the daily entry with the synced total" do
      ...
      it "preserves correct total (previous days + today)" do
    ...
    context "when the provider has fewer steps than manually entered" do
      before do
        user.step.add_steps(10_000, force: true)
        path_user.update_progress(active_path)
        stub_health_steps(5000)
      end
      ...
      it "still returns true and sets last sync" do
        service.call
        expect(user.reload.health_last_sync_at).to be_present
      end
    end

    context "when user is not connected to Google Health" do
      before { user.update!(health_uid: nil) }

      it "returns false without calling the API" do
        expect_any_instance_of(HealthClient).not_to receive(:fetch_steps)
        expect(service.call).to be false
      end

      it "logs the reason" do
        expect(Rails.logger).to receive(:warn).with(/no health_uid/)
        service.call
      end
    end
    ...
    context "when the provider returns zero steps" do
      before { stub_health_steps(0) }
    ...
    context "when the token has expired" do
      before do
        allow_any_instance_of(HealthClient).to receive(:fetch_steps)
          .and_raise(HealthClient::TokenRefreshError, "Token expired")
      end
    ...
    context "when the API returns an error" do
      before do
        allow_any_instance_of(HealthClient).to receive(:fetch_steps)
          .and_raise(HealthClient::ApiError, "API error")
      end
      ...
      it "logs the error" do
        expect(Rails.logger).to receive(:error).with(/sync failed.*ApiError/)
        service.call
      end
    end
    ...
    context "when catching up a previous day's steps" do
      ...
      it "adds only the delta when the provider has more steps than recorded" do
        allow_any_instance_of(HealthClient).to receive(:fetch_steps).with(yesterday).and_return(10_509)
      ...
      it "does not change steps when the provider returns the same count" do
        allow_any_instance_of(HealthClient).to receive(:fetch_steps).with(yesterday).and_return(6063)
